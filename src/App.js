...
function App() {
    ...
    (event) => {
      ...
      setTodos((todos) =>
        todos.map((todo) => {
          return {
            ...todo,
            done: todo.done || selectedId === todo.id.toString(),
          };
        })
      );
    },
  ...
  const handleUnDone = useCallback(
    (event) => {
      ...
      setTodos((todos) =>
        todos.map((todo) => {
          const unselectedItem = selectedItems.filter(
            (item) => item.getAttribute("data-key") === todo.id.toString()
          );
      ...
      <ui5-dialog header-text="Edit Todo item" ref={editDialog}>
        <div className="dialog-content">
          ...
          <div className="edit-wrapper date-edit-fields">
            ...
            <ui5-datepicker
              format-pattern="dd/MM/yyyy"
              value={todoBeingEditted.deadline}
              ref={dateEditInput}
            ></ui5-datepicker>
          </div>
        </div>
        <div className="dialog-footer">
          <ui5-button design="Transparent" ref={cancelBtn}>
            Cancel
          </ui5-button>
          {/*close dialog*/}
          <ui5-button design="Emphasized" ref={saveBtn}>
            Save
          </ui5-button>
          {/*save dialog info*/}
        </div>
